...
//         <h2 className="text-xl font-bold uppercase mb-4 text-center">
//           Categorías
//         </h2>
...
import { IoMdHelp } from "react-icons/io";
import { RiCouponLine } from "react-icons/ri";
import VentaArticulo from "./VentaArticulo";
import VentaCarro from "./VentaCarro";
...
const MainPage = ({ onLoginClick, userName = "Usuario" }) => {
  ...
  // Conexiones a las diferentes ventanas
  const navigateTo = {
    home: () => navigate("/"),
    ...
    VentaArticulo: () => navigate("/VentaArticulo"),
    VentaCarro: () => navigate("/VentaCarro"),
    logout: () => {
  ...
  // Resto de tus handlers originales
  const handleMouseEnter = () => {
    if (timeoutRef.current) {
      clearTimeout(timeoutRef.current);
      timeoutRef.current = null;
  ...
  // Efecto para manejar clicks fuera del menú (se mantiene igual)
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (menuRef.current && !menuRef.current.contains(event.target)) {
        setShowOpciones(false);
  ...
  // Componente MenuItem actualizado con las conexiones
  const MenuItem = ({ icon, text, onClick, highlight = false }) => (
    <button
      className={`w-full text-left px-4 py-3 text-sm ${
        highlight ? "text-red-600" : "text-gray-700"
  ...
  // Componente SearchBar (se mantiene igual)
  const SearchBar = () => (
    <div className="w-full max-w-3xl mx-auto mb-2">
      <form onSubmit={handleSearch} className="relative flex items-center">
        <input
  ...
  // Componente Header con las conexiones actualizadas
  const Header = () => (
    <header className="flex flex-col md:flex-row justify-between items-start p-6 md:p-6 bg-[#cae8ff] shadow-md">
      {/* Logo y barra de búsqueda (se mantiene igual) */}
      <div className="w-full flex flex-col md:flex-row justify-between items-center">
        <div className="cursor-pointer" onClick={navigateTo.home}>
          <img
            src={logoCompleto}
            alt="SOAP Logo"
            className="h-10 md:h-14 w-[150px]"
          />
        </div>
  ...
  // Componente Categorías (se mantiene igual)
  const Categorias = () => {
    const [showLeftButton, setShowLeftButton] = useState(false);
    const [showRightButton, setShowRightButton] = useState(true);
    const categoriesContainerRef = useRef(null);
  ...
  // Componente Anuncios (se mantiene igual)
  const Anuncios = () => (
    <section className="mb-12">
      <h2 className="text-center text-2xl font-bold mb-4">
        Anuncios de Temporada
  ...
  // Componente Footer (se mantiene igual)
  const Footer = () => (
    <footer className="py-8 flex justify-center items-center">
      <img
        src={logoLetras}
  ...
  // Render principal (se mantiene igual)
  return (
    <div className="min-h-screen bg-[#f4f6fc] relative text-[#484d45] max-w-screen overflow-x-hidden">
